...
bot.on('message', message => {
    ...
    if (message.content.startsWith('teach ale bot,')) {
        return _teachResponse(message.content.split('ale bot,')[1]);
    }
    
    if (message.content.startsWith('ale bot, joke')) {
        return _makeJoke(message);
    }
    
    if (message.content.startsWith('ale bot, play')) {
        return _playYoutubeStream(message);
    }
    
    if (!message.content.startsWith('ale bot,')) {
        return;
    }
    ...
    reply = _generateTextResponse(message.content.split('ale bot,')[1]);
...
function _playYoutubeStream(message) {
    ...
    const url = message.content.split('ale bot, play ')[1];
    const voiceChannel = message.member.voiceChannel;
    
    return voiceChannel.join()
        .then(connection => {
            return youtubeSearch.getYoutubeVideoURL(url)
                .then(resolvedURL => {
                    ...
                    return connection.playStream(ytdl(resolvedURL, { audioonly: true, quality: 'highestaudio' }));
                });
        })
        .then(dispatcher => {
